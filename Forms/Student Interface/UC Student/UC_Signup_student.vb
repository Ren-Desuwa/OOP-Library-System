...
Public Class UC_Signup_student
    ...
    Private Sub UC_Signup_student_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        img_hide.Visible = False ' [cite: 2, 3]
    End Sub
    ...
    Private Function IsUsernameTaken(username As String) As Boolean
        ' TODO: In the future, you will add your database lookup logic here. [cite: 7, 8]
        Return False
    End Function
    ...
    Private Sub txtBox_studentid_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtBox_studentid.KeyPress
        If e.KeyChar = ChrW(Keys.Back) Then Return ' Allow Backspace [cite: 10]

        Dim currentLength = txtBox_studentid.TextLength

        If currentLength < 8 Then ' 1. First 8 characters (MUST be numbers)
            If Not Char.IsDigit(e.KeyChar) Then e.Handled = True ' Reject non-numbers [cite: 11]

        ElseIf currentLength = 8 Then ' 2. Handle the 9th character (dash)
            If e.KeyChar <> "-" Then
                e.Handled = True ' Block if not a dash [cite: 12, 14]
            End If

        ElseIf currentLength = 9 Then ' 3. Handle the 10th character (MUST be a letter)
            If Not Char.IsLetter(e.KeyChar) Then
                e.Handled = True ' Reject non-letters [cite: 14]
            Else
                e.KeyChar = Char.ToUpper(e.KeyChar) ' It IS a letter, so convert to uppercase [cite: 15]
            End If

        Else ' 4. Max length is 10
            e.Handled = True ' Reject all other keys [cite: 15]
        End If
    End Sub
    ...
    Private Sub txtBox_studentid_TextChanged(sender As Object, e As EventArgs) Handles txtBox_studentid.TextChanged
        If isAddingDash OrElse isDeleting Then Return ' Exit if we are adding the dash or deleting [cite: 16]

        If txtBox_studentid.TextLength = 8 Then ' If 8 numbers were just typed
            isAddingDash = True ' Set flag
            txtBox_studentid.AppendText("-") ' Add dash [cite: 17]
            txtBox_studentid.SelectionStart = txtBox_studentid.TextLength ' Move cursor to end [cite: 18]
